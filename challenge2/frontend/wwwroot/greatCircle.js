import L from 'leaflet'; // exposed from global by browserify
import LatLonEllipsoidal from 'geodesy/latlon-ellipsoidal-vincenty.js';

var NORTH_POLE = new LatLonEllipsoidal(90, -180);
var SOUTH_POLE = new LatLonEllipsoidal(-90, 180);
...
L.GreatCircle = L.Polygon.extend({
    initialize: function (latlng, radius, options) {
        this._latlng = L.latLng(latlng);
        this._mRadius = Math.min(MAX_RADIUS_METERS, radius);
        var shape = this._computeShape(this._latlng, this._mRadius);

        L.Polygon.prototype.initialize.call(this, shape, options);
    },

    isInside: function (latlng) {
        return this._latlng.distanceTo(latlng) <= this._mRadius;
    },
    ...
    */
    _computeShape: function (center, radius) {
        var center = new LatLonEllipsoidal(center.lat, center.lng);
        var coords = [];
